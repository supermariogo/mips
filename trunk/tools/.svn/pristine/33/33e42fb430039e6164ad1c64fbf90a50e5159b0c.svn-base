...
##################################################################
   ...
   .ent	entry
entry:
   .set noreorder   # http://yuxu9710108.blog.163.com/blog/static/23751534201071922654972/

   #These four instructions should be the first instructions.
   #convert.exe previously initialized $gp, .sbss_start, .bss_end, $sp
   la    $gp, _gp             #initialize global pointer load address
   la    $5, __bss_start      #$5 = .sbss_start
   la    $4, _end             #$2 = .bss_end
   la    $sp, InitStack+488   #initialize stack pointer

$BSS_CLEAR:
   sw    $0, 0($5)            #sw Rs, off16(RT)------> MEM32(RT + OFF16¡À) = RS
   slt   $3, $5, $4           #slt RD, RS, RT-------->RD = (RS¡À < RT¡À) ? 1 : 0
   bnez  $3, $BSS_CLEAR
   addiu $5, $5, 4            # RD = RS + CONST16¡À

   jal   main                 #jal ADDR28  ----->RA = PC + 8; PC = PC31:28 :: ADDR28? 
   nop
$L1:
   j $L1
