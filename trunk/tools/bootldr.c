...
char *xtoa(unsigned long num)//16进制转ANSIC？
{
   static char buf[12];
   int i, digit;
   buf[8] = 0;
...
unsigned long getnum(void)  //获取输入的数据的真实值, 因为输入的是ansic 所以要转换
{
   int i;
   ...
   for(i = 0; i < 16; )
   {
      ...
      if('0' <= ch && ch <= '9')
         ...
         ch = ch - 'a' + 10;
      else if(ch == 8)  // ANSIC 8 --> BackSpace 
      {
         if(i > 0)
         {
            --i;
            putchar(ch);  //这个char就是BS
            putchar(' ');  //相当于擦成空白
            putchar(ch);   //再退回去
         }
         value >>= 4;
      ...
      putchar(ch2);
      value = (value << 4) + ch;  //   0x 1111 2222 3333 4444  最后4位， 5-8位，相当于个十百千
      ++i;
   }
   putchar('\r');
   putchar('\n');
...
int main(void)
{
   ...
   if((MemoryRead(GPIOA_IN) & 1) && (MemoryRead(FLASH_BASE) & 0xffff) == 0x3c1c)
   {
      ...
      funcPtr = (FuncPtr)RAM_EXTERNAL_BASE;//强制类型转换
      funcPtr();//jump！
   }
   for(;;)
   {
